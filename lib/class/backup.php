...
class Backup
{
	...
	function __construct($host = "localhost", $dbname = "'", $user = "your_username", $pass = "your_password")
	{
		$this->user = $user;
		$this->pass = $pass;
		$this->dbname = $dbname;
...
if($table == "geoip") continue;
		...
		/*
		 * USE EXAMPLE $connection = new MySql(SERVERHOST,"your_db_name",DBUSER, DBPASS); $connection->backup_tables(); //OR backup_tables("posts"); $connection->closeConnection();
		 */
	} /* end function */
	...
	{ /* save as utf8 encoding */
		...
		fclose($f);
		/*
		 * USE EXAMPLE this is only used by public function above... $this->writeUTF8filename($filename,$data);
		 */
	} /* end function */
	...
	public function recoverDB($file_to_load)
	{
		...
		/*
		 * USE EXAMPLE this is only used by public function above... recoverDB("some_buck_up_file.sql");
		 */
	} /* end function */
