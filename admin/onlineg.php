...
$page_title = 'گزارش مهمانان آنلاین';
?>
<?php require_once $path.'theme/header.php';?>
<h1>گزارش مهمانان آنلاین</h1>
<table style="width: 100%">
	...
	$count = "SELECT COUNT(*) as count FROM `dlinfo_guest` WHERE `time`>$time";
	$count = $database->query($count)->fetch();
	$page = new Pagination($_GET['page'], $setting['pagelimit'], $count['count']);
	
	$users = $database->query("SELECT * FROM `dlinfo_guest` WHERE `time`>$time LIMIT $setting[pagelimit] OFFSET {$page->offset()}");
